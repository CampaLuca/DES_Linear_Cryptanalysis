...
DES::DES(int mode, ui64 key)
{
    if (mode == 0)  // mode 0 means from 64 bit key
        keygen(key);
    else
        keygen_56bits(key);
}

void DES::keygen_from56bits(ui64 key) {
    keygen_56bits(key);
}
...
void DES::keygen_56bits(ui64 key)
{
